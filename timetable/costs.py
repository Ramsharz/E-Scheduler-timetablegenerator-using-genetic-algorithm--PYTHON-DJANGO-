...
def empty_space_teachers_cost(teachers_empty_space):
    ...
    for teacher_name, times in teachers_empty_space.items():
        ...
        for i in range(1, len(times) - 1):
            ...
            b = times[i]
            diff = b - a
            # classes are in the same day if their time div 6 is the same
            if a // 6 == b // 6 and diff > 1:
                empty_per_day[a // 6] += diff - 1
                cost += diff - 1
...
def free_hour(matrix):
    ...
    if exists:
        return 'break exists for whole week from 12:00 to 01:00'
                    ...
                    if next_field is not None:
                        ...
                        for g in g1:
                            if g in g2:
                                cost_group += 1
                                cost_class[field] += 1
                            #print('Teacher cost', cost_teacher)
                            #print('Class cost', cost_class)
                            #print('Classroom cost', cost_classrooms)
