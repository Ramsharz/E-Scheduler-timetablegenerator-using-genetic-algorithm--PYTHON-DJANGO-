...
from .models import Resource, Staff, Schedule, Course
...
def load_data(file_path, teachers_empty_space, groups_empty_space, subjects_order):
    ...
    for cl in data['Classes']:
        ...
        new = Class(new_group, new_teacher,
                    cl['Subject'], cl['Type'], cl['Duration'], cl['Classroom'])
        # add groups
...
def set_up(num_of_columns):
    ...
    :return: matrix, free
    """
    col, row = num_of_columns, 30                                          # 5 (workdays) * 6 (work hours) = 30
...
def write_solution_to_file(matrix, data, filled, filepath, groups_empty_space, teachers_empty_space, subjects_order):
    ...
    for class_index, times in filled.items():
        ...
        f.write('\nClassroom: {:2s}\nTime: {}'.format(
            room[:room.rfind('-')], days[times[0][0] // 6]))
        t = []
        for time in times:
            h = hours[time[0] % 6]
            if h == 13:
        ...
        obj = Schedule.objects.create(
            teacher=c.teacher, subject=c.subject, semester=int(list(groups[:len(groups) - 2].replace(' ', ''))[0]), section=list(groups[:len(groups) - 2].replace(' ', ''))[1], type=c.type, time_slot=t, room=room[:room.rfind('-')], day=days[times[0][0] // 6], duration=c.duration)
    f.close()
